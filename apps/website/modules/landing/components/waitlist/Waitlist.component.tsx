  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/services/sendWaitlist`,
        {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row w-full h-full items-center">
      <div className="relative w-full flex flex-col items-center justify-center md:w-1/2 lg:w-1/2 xl:w-1/2 h-[300px] md:h-[500px] lg:h-[500px] xl:h-[500px]">
        ...
        <h1 className="!text-primary font-medium text-4xl">For Business</h1>
      </div>
      {isSent && (
        <div className="fade-in w-11/12 md:w-1/2 lg:w-1/4 xl:w-1/4 flex m-auto justify-center items-center flex-col">
          <p className="text-center">
            You're on the list. We'll contact you soon with updates. Follow us
      ...
      {!isSent && (
        ...
        >
          ...
          <div className="flex flex-row items-center gap-2">
            <h1 className="text-2xl font-bold">Join our waiting list</h1>
            <Popover>
              <PopoverTrigger>
                <Info strokeWidth="2px" className="cursor-pointer" />
              </PopoverTrigger>
              <PopoverContent>
                <b>Lattter For Business</b> is currently by invitation only ●
          ...
          <p className="my-4 text-center">
            For Business has features such as custom AI models, teams, shared
            goals and more.
          </p>
          ...
          {isSent === false && (
            <Button
              ...
              type="submit"
              disabled={
                waitlistInfo.email.length > 0 && waitlistInfo.company.length > 0
                  ? false
                  : true
              }
            >
