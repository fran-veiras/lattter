...
import { getFolderId } from "~background"
...
function IndexPopup() {
  ...
  const saveLink = async () => {
    if (currentUrl) {
      ...
      if (response.ok) {
        Array.isArray(savedItemsParsed) && savedItemsParsed.push(currentUrl)
      ...
      <div className="flex w-full flex-col">
        ...
        <div className="p-4 w-full h-full">
          {user === USER_STATES.LOADING && (
            <h2 className="text-lg font-light">Cargando usuario...</h2>
          )}
          {user === USER_STATES.NOT_FOUND && (
            <>
              <h2 className="text-lg font-light my-2">
                Your AI research and study co-pilot, ensuring your compliance.
              </h2>
