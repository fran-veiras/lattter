...
const addSaveButton = async () => {
  ...
  tweets.forEach(async (tweet) => {
    ...
    if (!tweet.querySelector(".save-button")) {
      ...
      // Bookmark link.
      const a = tweet.querySelector('a[href*="status"]') as HTMLAnchorElement
      ...
      if (targetDiv) {
        ...
        if (!tweet.querySelector(".save-button")) {
          ...
          const savingItemListener = async (request, sender, sendResponse) => {
            ...
            if (request.action === "savingItem" && request.href === a.href) {
