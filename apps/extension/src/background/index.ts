...
// returns the folder id, if it exists, otherwise creates. Is required to create an item.
export async function getFolderId(sessionParsed, folderName) {
  const folderIdResponse = await fetch(
    `${
      process.env.PLASMO_PUBLIC_URL?.includes("localhost")
        ? "http://localhost:3000"
        : "https://www.lattter.com"
    }/services/folders/getByName?name=${folderName}`,
    {
      method: "GET",
      headers: {
  ...
  if (!folderIdResponse.ok) {
    ...
    if (!response.ok) {
      console.error("Failed to fetch folder ID:", folderIdResponse.statusText)
      return null
    }

    const folderId = response.json()
    return folderId
  }

  const folderId = await folderIdResponse.json()
  return folderId
}
  ...
  if (request.action === "saveItem") {
    try {
      ...
      if (sessionParsed?.access_token) {
        ...
        const folderId = await getFolderId(sessionParsed, folderName)
